...
一款面向全年龄的一笔画变种解谜游戏。屏幕上是散落的花朵，用手指一笔连起所有**同色**花朵（连线不能交叉、不能重复经过同一朵花），连完所有颜色即可过关。
...
4. 连线一旦与已有线段交叉，本次吸附会被拒绝。
5. 顶部色块显示每组颜色的完成进度；连完所有颜色过关。
...
设计约束：单色关的花朵处于凸位置（沿外圈连线即解），多色关各组位于互不相交区域或严格嵌套，从而保证每个颜色都存在不交叉的一笔画解。新增关卡后，`test/levels_test.dart` 会自动校验可解性。

## 核心算法

- **线段相交**：`segmentsCross` 用方向判定（`_orient` 叉积符号）判断跨立，共线重叠视为相交，共享端点不算相交。
- **可解性求解**：`solveNonCrossingPath` 对 ≤ 8 个点暴力枚举全排列，返回第一条不交叉的哈密顿路径，用于“提示”与关卡校验。