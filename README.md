...
2. 部分关卡的花朵之间有“预设道路”：只有存在道路的两朵花才能直接相连，不能凭空搭桥。
3. 一次手势只能连同一颜色；抬手指后可换颜色继续。
4. 接续某颜色时，需从该颜色当前路径的“头部”（发光的那朵）开始。
5. 连完该颜色所有花后，需再拖回**起点那朵花**，把首尾连起来才算完成该颜色。
6. 连线一旦与已有线段交叉，本次吸附会被拒绝。
7. 顶部色块显示每组颜色的完成进度；连完所有颜色过关。
...
设计约束：单色关的花朵处于凸位置（沿外圈连线再回到起点即解），多色关各组位于互不相交区域或严格嵌套，从而保证每个颜色都存在不交叉的首尾相连闭环解。高级关卡通过 `FlowerGroup` 的 `edges`（预设道路）限制可连线，从而把“沿外圈一圈”变成需要规划的图论难题。新增关卡后，`test/levels_test.dart` 会自动校验可解性。

## 核心算法

- **线段相交**：`segmentsCross` 用方向判定（`_orient` 叉积符号）判断跨立，共线重叠视为相交，共享端点不算相交。
- **可解性求解**：`solveNonCrossingCycle` 对 ≤ 8 个点暴力枚举全排列，返回第一条不交叉且满足预设道路（`edges`）的哈密顿回路（首尾相连），用于“提示”与关卡校验。